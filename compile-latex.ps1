...
if ($LASTEXITCODE -eq 0) {
    ...
    $errors = Select-String -Path "$TexFile.log" -Pattern "! LaTeX Error"
    if ($errors) {
        Write-Host "      X Terdeteksi ERROR! Cek log." -ForegroundColor Red
    } else {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "      ! Selesai dengan warning (Bibliografi)" -ForegroundColor Yellow
}
